...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, latents):
        ...
        return (
            quantized,
            loss,
            encoding_indices.view(latents_shape[0], latents_shape[1], latents_shape[2]),
        )
...
class VisualTokenizer(nn.Module):
    ...
    def __init__(self, config: VisualConfig):
        ...
        if config.use_progressive_tokenizer:
            ...
            self.quantizers = nn.ModuleList(
                [BitwiseQuantizer(bits=config.tokenizer_bits, embedding_dim=256)]
            )
    ...
    def decode(
        self, indices: torch.Tensor, shape: Tuple[int, int] = None
    ) -> torch.Tensor:
        """
        ...
        z_q = (
            self.quantizers[-1].vq.embedding(indices).permute(0, 3, 1, 2)
        )  # [B, C, H, W]
        x_recon = self.decoder(z_q)
        return x_recon
