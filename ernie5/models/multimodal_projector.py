...
class MultiModalProjector(nn.Module):
    ...
    def __init__(self, config: ERNIE5Config):
        ...
        self.visual_encoder = DualPathVisualEncoder(config.visual, out_dim=self.hidden_size)
        
        # 2. Audio Projector (Encoder + Linear)
        self.audio_encoder = AudioDepthwiseEncoder(config.audio, out_dim=self.hidden_size)
