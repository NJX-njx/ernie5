...
class DualPathVisualEncoder(nn.Module):
    ...
    def __init__(self, config: VisualConfig, out_dim: int):
        ...
        self.vit_blocks = nn.ModuleList([
            nn.TransformerEncoderLayer(d_model=config.vit_hidden_dim, nhead=8, dim_feedforward=config.vit_hidden_dim*4, batch_first=True)
            for _ in range(4)
        ])
        ...
        self.fusion_attn = nn.MultiheadAttention(embed_dim=config.vit_hidden_dim, num_heads=8, batch_first=True)
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        combined_tokens = torch.cat([vit_tokens, cnn_tokens], dim=1) # [B, N_total, D_vit]
        
        # Fusion Attention
        # 这里的Fusion在论文中是一个"Patch Merger with Attention"
        # 简单起见，我们对拼接后的序列做一次Self-Attention
        fused_tokens, _ = self.fusion_attn(combined_tokens, combined_tokens, combined_tokens)
