...
from ernie5.models.transformer import RMSNorm, TransformerBlock
from ernie5.models.visual_generator import VisualGenerator
from ernie5.training.elastic import ElasticContext, ElasticTrainingManager
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            position_ids = torch.zeros(
                (batch_size, seq_len, 3), dtype=torch.long, device=hidden_states.device
            )
...
class ERNIE5ForCausalLM(nn.Module):
    ...
    def __init__(self, config: ERNIE5Config):
        ...
        self.lm_head = nn.Linear(
            config.hidden_size, config.get_unified_vocab_size(), bias=False
        )
        ...
        **kwargs
        ...
        if visual_labels is not None:
            visual_logits, visual_loss = self.visual_head(
                hidden_states, labels=visual_labels, loss_mask=visual_mask
            )
        ...
        if audio_labels is not None:
            _, audio_loss = self.audio_head(
                hidden_states, labels=audio_labels, loss_mask=audio_mask
            )
        ...
        loss = None
        if (
            text_loss is not None
            or visual_loss is not None
            or audio_loss is not None
            or mtp_loss is not None
        ):
            # NOTE:
            ...
            loss_terms = [
                term
                for term in (text_loss, visual_loss, audio_loss)
                if term is not None
            ]
            loss = (
                sum(loss_terms)
