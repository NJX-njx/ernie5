...
class MultiModalCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for sample in batch:
            ...
            for item in content:
                if item["type"] == "text":
                    ...
                    audio_label_tokens.extend([[-100] * self.config.audio.rvq_layers] * seq_len)
                    t_cursor += seq_len

                elif item["type"] == "image":
                    if self.visual_tokenizer is None:
                        raise ValueError("visual_tokenizer is required for image inputs")
                    ...
                    sample_pos.append(torch.tensor([[t_cursor, 0, 0]], dtype=torch.long))
                    t_cursor += 1

                    tokens_per_scale = self.visual_tokenizer.encode_multiscale(item["value"].unsqueeze(0))
                    for scale_tokens in tokens_per_scale:
                        ...
                        audio_label_tokens.extend([[-100] * self.config.audio.rvq_layers] * (h * w))
                        group_counter += 1
                    ...
                    audio_label_tokens.extend(codes[:, :seq_len].transpose(0, 1).tolist())
                    t_cursor += seq_len
            ...
            if len(sample_ids) > self.config.max_position_embeddings:
                ...
                token_modalities = token_modalities[:self.config.max_position_embeddings]
                visual_label_tokens = visual_label_tokens[:self.config.max_position_embeddings]
                audio_label_tokens = audio_label_tokens[:self.config.max_position_embeddings]

            input_ids_list.append(torch.tensor(sample_ids, dtype=torch.long))

            visual_labels_list.append(torch.tensor(visual_label_tokens, dtype=torch.long))
            ...
            # modality masks
            text_mask_list.append(torch.tensor([m == "text" for m in token_modalities], dtype=torch.bool))
            visual_mask_list.append(torch.tensor([m == "visual" for m in token_modalities], dtype=torch.bool))
            audio_mask_list.append(torch.tensor([m == "audio" for m in token_modalities], dtype=torch.bool))
        ...
        text_mask = torch.nn.utils.rnn.pad_sequence(text_mask_list, batch_first=True, padding_value=False)
