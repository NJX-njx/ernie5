...
class ERNIE5Config:
    ...
    def get_num_parameters(self, include_moe: bool = True) -> int:
        ...
        if include_moe:
            expert_params = 2 * self.hidden_size * int(self.intermediate_size * self.moe.expert_ffn_multiplier)
            total_params += moe_layers * (attn_params + self.moe.num_experts * expert_params)
        else:
